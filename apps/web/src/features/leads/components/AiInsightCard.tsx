...
export function AiInsightCard({ lead }: { lead: LeadDetail }) {
  ...
  const errorCode = (aiInsight.error as any)?.response?.data?.error?.code as string | undefined;
  const errorMessage = (aiInsight.error as any)?.response?.data?.error?.message as string | undefined;
