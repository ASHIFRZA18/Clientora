...
import { getGoogleProfile } from "../../lib/google.js";
...
export const authService = {
  ...
  async loginWithGoogle(code: string, ip?: string) {
    ...
    if (!user) {
      ...
      if (existingByEmail) {
        // Existing email/password account — link Google to it instead of creating a duplicate.
        user = await authRepository.linkGoogleId(existingByEmail.id, profile.googleId);
      } else {
